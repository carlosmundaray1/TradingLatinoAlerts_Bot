...
W_HMM_REGIME: int = 15     # Optimo: 49.8% deteccion (vs 37.6% baseline)
    ...
    for i in range(len(df)):
        ...
        if regime_changed[i]:
            ...
            if nb == "bullish":
                ...
                hmm_score = 0
                debug_parts.append('HMM_NEUTRAL+0')
        ...
        # --- Componente 4: Alignment regimen-senal (SOLO si hubo cambio reciente) ---
        alignment_score_long = 0
        alignment_score_short = 0
        if regime_changed[i] and "regime" in df.columns:
