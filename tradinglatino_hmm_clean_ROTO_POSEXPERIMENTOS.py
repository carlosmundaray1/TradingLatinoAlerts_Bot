...
SIGNAL_SCORE_THRESHOLD: int = 60   # Score mínimo (0-100) para activar la señal (Versión D: calibrado final)
...
W_HMM_REGIME: int = 20     # Optimo: 49.8% deteccion (vs 37.6% baseline)
...
HYBRID_CONFIDENCE_THRESHOLD: int = 40  # Confianza minima (0-100) para activar alerta
...
# -- VENTANA DESLIZANTE HMM: Re-entrenar cada N velas para adaptarse al mercado actual --
# Tamano de ventana para entrenar el HMM (en velas)
# Valores recomendados por timeframe:
#   1h: 1500 velas (~2 meses)
#   4h: 1000 velas (~6 meses)
#   1d: 500 velas (~1.5 anos)
#   1wk: 300 velas (~6 anos)
HMM_SLIDING_WINDOW: int = 500
# Cada cuantas velas re-entrenar (stride). Menor = mas actualizado pero mas lento.
HMM_SLIDING_STRIDE: int = 50
...
def _classify_regime_bias(description: str) -> str:
    ...
    - ACUMULACION se clasifica como bullish (preparacion para movimiento alcista)
    - ALTA VOLATILIDAD/VOLATILIDAD NEUTRA/LATERAL se mantienen neutrales"""
    d = description.upper()
    if any(x in d for x in ["ALCISTA", "EXPANSION ALCISTA", "TREND ALCISTA", "ACUMULACION"]):
        return "bullish"
...
def build_hmm_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "rsi14" in df.columns:
        features["rsi_norm"] = (df["rsi14"] - 50) / 50  # Normalizado: -1 a +1
    ...
    # -- MEJORA HMM: Features direccionales de indicadores TradingLatino existentes --
    # DI spread: (plus_di - minus_di) normalizado a [-1, 1]
    if "plus_di" in df.columns and "minus_di" in df.columns:
        di_sum = (df["plus_di"] + df["minus_di"]).replace(0, np.nan)
        features["di_spread"] = ((df["plus_di"] - df["minus_di"]) / di_sum).fillna(0)
    # EMA fast/slow spread normalizado por ATR (ya existe ema_spread_atr, anadimos raw version)
    if "ema_fast" in df.columns and "ema_slow" in df.columns:
        features["ema_spread_raw"] = (df["ema_fast"] - df["ema_slow"]) / df["Close"].replace(0, np.nan) * 100
        features["ema_spread_raw"] = features["ema_spread_raw"].fillna(0).clip(-5, 5)
    # Signal score net: signal_score_long - signal_score_short (direccion neta)
...
def fit_hmm(features_df: pd.DataFrame) -> Tuple[Any, np.ndarray, pd.DataFrame, pd.DataFrame, Optional[np.ndarray], np.ndarray, np.ndarray, np.ndarray]:
    """Ajusta HMM probando varios estados, elige el mejor por BIC.
    Retorna: model, states, state_summary, bic_df, trans_mat, probas, scaler_means, scaler_stds"""
    clean = features_df.dropna()
    if len(clean) < 100:
        print("  ERROR: Datos insuficientes para HMM.")
        return None, np.array([]), pd.DataFrame(), pd.DataFrame(), np.array([]), np.array([]), np.array([]), np.array([])
    X = clean.values
    ...
    # -- MEJORA: Suavizar estados HMM para eliminar cambios espurios de regimen --
    best_states = _smooth_states(best_states, min_duration=2)

    # -- MEJORA HMM: Calcular probabilidades de estado (predict_proba) para scoring dinamico --
    best_probas = best_model.predict_proba(X_scaled)
    ...
    trans_mat = reordered
    return best_model, best_states, state_summary, bic_df, trans_mat, best_probas, means, stds

# -- VENTANA DESLIZANTE HMM: Re-entrenar en ventanas para adaptarse al mercado --
def fit_hmm_sliding(features_df: pd.DataFrame,
                   window_size: Optional[int] = None,
                   stride: Optional[int] = None) -> Tuple[Any, np.ndarray, pd.DataFrame, pd.DataFrame, Optional[np.ndarray], np.ndarray, np.ndarray, np.ndarray]:
    """
    HMM con ventana deslizante. Entrena un HMM en ventanas superpuestas de datos
    para que el modelo se adapte a las condiciones actuales del mercado.
    
    Para cada ventana:
      1. Toma los ultimos window_size velas
      2. Entrena HMM en esa ventana
      3. Predice estados para las ultimas  velas de la ventana
      4. Avanza stride velas y repite
    
    Args:
        features_df: DataFrame con features para HMM
        window_size: Tamano de ventana de entrenamiento (default: HMM_SLIDING_WINDOW)
        stride: Cada cuantas velas re-entrenar (default: HMM_SLIDING_STRIDE)
    
    Retorna:
        Tuple con (model, states, state_summary, bic_df, trans_mat, probas, means, stds)
        - states: Array de estados para TODO el dataset
        - state_summary: Resumen del ultimo (mas reciente) modelo
        - trans_mat: Matriz de transicion del ultimo modelo
    """
    if window_size is None:
        window_size = HMM_SLIDING_WINDOW
    if stride is None:
        stride = HMM_SLIDING_STRIDE
    
    clean = features_df.dropna()
    n_total = len(clean)
    
    if n_total <= window_size:
        # No hay suficientes datos para sliding window, usar fit_hmm normal
        return fit_hmm(features_df)
    
    window_size = min(window_size, n_total)
    stride = min(stride, window_size // 3)  # Al menos 3 ventanas
    if stride < 10:
        stride = max(10, window_size // 10)
    
    # Generar puntos de inicio de cada ventana
    starts = list(range(0, n_total - window_size + 1, stride))
    if starts[-1] + window_size < n_total:
        starts.append(n_total - window_size)
    
    max_states = max(HMM_STATE_RANGE)  # 5
    all_states = np.zeros(n_total, dtype=int)
    all_probas = np.zeros((n_total, max_states))
    
    last_state_summary = pd.DataFrame()
    last_trans_mat = None
    last_model = None
    last_bic_df = pd.DataFrame()
    last_means = np.array([])
    last_stds = np.array([])
    
    for wi, start in enumerate(starts):
        end = min(start + window_size, n_total)
        features_window = features_df.iloc[start:end]
        
        # Entrenar HMM en esta ventana
        result = fit_hmm(features_window)
        model, states, state_summary, bic_df, trans_mat, probas, means, stds = result
        
        if model is None or len(states) == 0:
            continue
        
        if wi == 0:
            # Primera ventana: asigna TODOS los estados de la ventana
            n_assign = min(len(states), window_size)
            all_states[:n_assign] = states[:n_assign]
            if probas.ndim > 1:
                n_p = min(probas.shape[0], n_assign)
                n_c = min(probas.shape[1], max_states)
                all_probas[:n_p, :n_c] = probas[:n_p, :n_c]
        else:
            # Ventanas siguientes: asigna SOLO las ultimas  velas
            tail_size = min(stride, len(states))
            global_start = start + window_size - stride
            global_end = min(global_start + tail_size, n_total)
            
            if global_end > global_start:
                n_assign = global_end - global_start
                s_start = max(0, len(states) - n_assign)
                s_end = s_start + n_assign
                
                all_states[global_start:global_end] = states[s_start:s_end]
                
                if probas.ndim > 1:
                    p_start = max(0, probas.shape[0] - n_assign)
                    p_end = min(p_start + n_assign, probas.shape[0])
                    n_p = p_end - p_start
                    n_c = min(probas.shape[1], max_states)
                    if n_p > 0:
                        all_probas[global_start:global_start+n_p, :n_c] = probas[p_start:p_end, :n_c]
        
        # Mantener metadatos de la ventana mas reciente
        last_state_summary = state_summary
        last_trans_mat = trans_mat
        last_model = model
        last_bic_df = bic_df
        last_means = means
        last_stds = stds
    
    return last_model, all_states, last_state_summary, last_bic_df, last_trans_mat, all_probas, last_means, last_stds
...
# -- MEJORA 1A: Score HMM proporcional a probabilidad del regimen (predict_proba) --
def compute_signal_scores_with_hmm(df, state_summary, probas, trans_mat=None):
    """Anade peso HMM proporcional a predict_proba + persistencia de matriz de transicion.
    - Score boost = W_HMM_REGIME * P(estado alcista/bajista) * persistence_factor
    - persistence_factor se ajusta segun la estabilidad de la matriz de transicion:
      regimenes estables (diagonal alta) = mayor peso, regimenes inestables = menor peso."""
    ...
    for _, r in state_summary.iterrows():
        ...
        if bias == "bullish":
            bullish_states.append(state)
        elif bias == "bearish":
            bearish_states.append(state)
    
    # Probabilidad de estar en estados alcistas/bajistas (predict_proba)
    n = min(len(probas), len(df))
    prob_bullish = np.zeros(len(df))
    prob_bearish = np.zeros(len(df))
    n_states_val = probas.shape[1] if probas.ndim > 1 and probas.shape[0] > 0 else 0
    
    if n_states_val > 0:
        if bullish_states:
            for s in bullish_states:
                if s < n_states_val:
                    prob_bullish[:n] += probas[:n, s]
        if bearish_states:
            for s in bearish_states:
                if s < n_states_val:
                    prob_bearish[:n] += probas[:n, s]
    
    # Persistence factor desde la matriz de transicion
    persistence_factor = 1.0
    if trans_mat is not None and len(trans_mat) > 0:
        # Promedio de la diagonal = probabilidad media de permanecer en el mismo estado
        avg_stability = np.mean([trans_mat[s, s] for s in range(len(trans_mat))])
        # Mapear: 0.0 -> 0.6, 0.5 -> 0.9, 1.0 -> 1.2
        persistence_factor = 0.6 + 0.6 * avg_stability
        persistence_factor = np.clip(persistence_factor, 0.5, 1.3)
    
    # Score boost proporcional: W_HMM_REGIME * prob * persistence
    effective_weight = W_HMM_REGIME * persistence_factor
    
    if "signal_score_long" in df.columns:
        # Clip prob_bullish a max 1.0 antes de multiplicar
        boost_long = np.clip(prob_bullish, 0, 1.0) * effective_weight
        df["signal_score_long"] = df["signal_score_long"].astype(float) + boost_long
    if "signal_score_short" in df.columns:
        boost_short = np.clip(prob_bearish, 0, 1.0) * effective_weight
        df["signal_score_short"] = df["signal_score_short"].astype(float) + boost_short
    
    # Recalcular senales con filtro de velas consecutivas
    if "signal_score_long" in df.columns and "signal_score_short" in df.columns:
        raw_long = (df["signal_score_long"] >= SIGNAL_SCORE_THRESHOLD).astype(bool)
        raw_short = (df["signal_score_short"] >= SIGNAL_SCORE_THRESHOLD).astype(bool)
        df["signal_long"] = _consecutive_bars_filter(raw_long, MIN_CONSECUTIVE_BARS)
        df["signal_short"] = _consecutive_bars_filter(raw_short, MIN_CONSECUTIVE_BARS)
    return df
...
#         smi_pos_falling = smi_rising & (df["smi_hist"] > 0) & (df["smi_hist"] < 0.5)
    ...
    if "adx_delta" in df.columns and "adx" in df.columns:
        ...
        if "plus_di" in df.columns and "minus_di" in df.columns:
            ...
            df["precursor_adx_long"] = pd.Series(False, index=df.index)
            df["precursor_adx_short"] = pd.Series(False, index=df.index)
    ...
    long_cols = [c for c in df.columns if c.startswith("precursor_") and "long" in c.lower() and "confidence" not in c.lower()]
    short_cols = [c for c in df.columns if c.startswith("precursor_") and "short" in c.lower() and "confidence" not in c.lower()]
...
def compute_hybrid_alert(df: pd.DataFrame, states: np.ndarray, state_summary: pd.DataFrame,
                            probas: Optional[np.ndarray] = None,
                            trans_mat: Optional[np.ndarray] = None) -> pd.DataFrame:
    """
    Sistema hibrido SIMPLIFICADO que usa predict_proba en lugar de cambios duros de regimen.
    
    Para cada vela:
      1. HMM_PROBA: P(estado alcista/bajista) * HYBRID_W_HMM (0-30 continuo)
      2. PRECURSOR: Componentes precursoras activas en ventana (0-35)
      3. ALIGNMENT: Alineacion regimen actual * prob (0-15 continuo)
      4. PERSISTENCE: Factor de persistencia desde trans_mat (multiplicador 0.8-1.2)
    """
    ...
    for _, r in state_summary.iterrows():
        s = int(r["state"])
        bias = _classify_regime_bias(r["description"])
        state_bias_map[s] = bias
        if bias == "bullish":
            bullish_states.append(s)
        elif bias == "bearish":
            bearish_states.append(s)

    # Persistence factor desde trans_mat
    persistence_factor = 1.0
    if trans_mat is not None and len(trans_mat) > 0:
        avg_stability = np.mean([trans_mat[s, s] for s in range(len(trans_mat))])
        persistence_factor = 0.8 + 0.4 * avg_stability  # 0.8 a 1.2

    # Probabilidad de regimen alcista/bajista desde predict_proba
    n = min(len(probas), len(df)) if probas is not None and len(probas) > 0 else 0
    prob_bullish = np.zeros(len(df))
    prob_bearish = np.zeros(len(df))
    
    if n > 0 and probas.ndim > 1:
        n_states_val = probas.shape[1]
        if bullish_states:
            for s in bullish_states:
                if s < n_states_val:
                    prob_bullish[:n] += probas[:n, s]
    ...
    # Precursores en ventana
    precursor_any_long = pd.Series(False, index=df.index)
    precursor_any_short = pd.Series(False, index=df.index)
    if "precursor_long" in df.columns:
        precursor_any_long = df["precursor_long"].rolling(HYBRID_LOOKBACK + 1, min_periods=1).max().astype(bool)
    if "precursor_short" in df.columns:
        precursor_any_short = df["precursor_short"].rolling(HYBRID_LOOKBACK + 1, min_periods=1).max().astype(bool)

    # Calcular componentes para cada vela (vectorizado)
    for i in range(len(df)):
        # Componente 1: HMM probabilidad (0-30 continuo)
        pb = np.clip(prob_bullish[i], 0, 1.0) if i < n else 0
        ps = np.clip(prob_bearish[i], 0, 1.0) if i < n else 0
        hmm_long = pb * HYBRID_W_HMM * persistence_factor
        hmm_short = ps * HYBRID_W_HMM * persistence_factor

        # Componente 2: Precursor (0-35)
        prec_long = HYBRID_W_PRECURSOR if precursor_any_long.iloc[i] else 0
        prec_short = HYBRID_W_PRECURSOR if precursor_any_short.iloc[i] else 0

        # Componente 3: Alignment (0-15, ponderado por prob)
        align_long = pb * HYBRID_W_ALIGNMENT
        align_short = ps * HYBRID_W_ALIGNMENT

        # Confianza total
        conf_long = min(100, hmm_long + prec_long + align_long)
        conf_short = min(100, hmm_short + prec_short + align_short)
...
def main():
    ...
    for tf in TIMEFRAMES:
        ...
        try:
            ...
            # 3) HMM - features + entrenamiento con ventana deslizante
            t0 = time.time()
            features_df = build_hmm_features(df)
            # Usar ventana deslizante para adaptarse al mercado actual
            # Window size segun el timeframe
            window_map = {"1h": 1500, "4h": 1000, "1d": 500, "1wk": 300}
            sw = window_map.get(timeframe, HMM_SLIDING_WINDOW)
            model_hmm, states, state_summary, _, trans_mat, probas, _, _ = fit_hmm_sliding(features_df, window_size=sw)
            n_states = state_summary["state"].nunique() if not state_summary.empty else 0
            print(f"  HMM entrenado (sliding window={sw}): {n_states} estados ({time.time()-t0:.1f}s)")

            # -- MEJORA 1A: Score HMM proporcional a predict_proba + persistencia --
            df = compute_signal_scores_with_hmm(df, state_summary, probas, trans_mat)

            # -- ENFOQUE A: Detectar senales precursoras de cambios de tendencia --
            df = compute_precursor_signals(df)

            # -- SISTEMA HIBRIDO FINAL: HMM + Precursores con Pesos --
            df = compute_hybrid_alert(df, states, state_summary, probas, trans_mat)
