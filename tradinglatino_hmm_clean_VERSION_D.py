...
def verify_signals_historically(df: pd.DataFrame, timeframe: str) -> Dict[str, Any]:
    ...
    tp_base = TAKE_PROFIT_PCT.get(timeframe, TAKE_PROFIT_PCT.get("1d", 2.0))
    max_bars = MAX_BARS_BY_TF.get(timeframe, 14)
        ...
        if df["signal_long"].iloc[i]:
            ...
            for j in range(len(close_prices)):
                if close_prices[j] == max_price:
                    bars_to_max = j + 1
                    break
            # TP dinámico basado en ATR (2x ATR) o TP fijo, el que sea mayor
            atr_entry = float(df["atr"].iloc[i]) if "atr" in df.columns else 0
            tp_atr = (atr_entry / entry_price * 100) * TP_ATR_MULT if atr_entry > 0 else 0
            tp_target = max(tp_base, tp_atr)
            ...
            long_results.append({
                ...
                "won": won,
                "tp_used": round(tp_target, 2),
                "tp_atr_based": round(tp_atr, 2),
                "tp_base": tp_base,
...
def verify_with_trailing_stop(df: pd.DataFrame, timeframe: str, trail_pct: float = 50.0) -> Dict[str, Any]:
    ...
    for i in range(len(df) - max_bars - 1):
        ...
        if df["signal_long"].iloc[i]:
            ...
            # Estrategia 1: TP FIJO + ATR dinámico
            max_price = high_prices.max()
            final_price = close_prices[-1]
            max_return_tp = (max_price - entry_price_f) / entry_price_f * 100.0
            final_return_tp = (final_price - entry_price_f) / entry_price_f * 100.0
            # TP dinámico basado en ATR (2x ATR) o TP base, el mayor
            atr_entry = float(df["atr"].iloc[i]) if "atr" in df.columns else 0
            tp_atr = (atr_entry / entry_price_f * 100) * TP_ATR_MULT if atr_entry > 0 else 0
            tp_target = max(tp_base, tp_atr)
            won_tp = max_return_tp >= tp_target

            # Estrategia 2: TRAILING STOP (retroceso desde maximo + ATR dinámico)
            trail_atr = (atr_entry / entry_price_f * 100) * TRAIL_ATR_MULT if atr_entry > 0 else 50.0
            trail_pct_dynamic = max(trail_pct, trail_atr)
        ...
        if df["signal_short"].iloc[i]:
            ...
            # Estrategia 1: TP FIJO + ATR dinámico para SHORT
            min_price = low_prices.min()
            final_price = close_prices[-1]
            max_return_tp = (entry_price_f - min_price) / entry_price_f * 100.0
            final_return_tp = (entry_price_f - final_price) / entry_price_f * 100.0
            # TP dinámico basado en ATR
            atr_entry_s = float(df["atr"].iloc[i]) if "atr" in df.columns else 0
            tp_atr_s = (atr_entry_s / entry_price_f * 100) * TP_ATR_MULT if atr_entry_s > 0 else 0
            tp_target = max(tp_base, tp_atr_s)
            won_tp = max_return_tp >= tp_target
...
def _make_price_chart(df: pd.DataFrame, states: np.ndarray, asset: str, timeframe: str, signal_info: Dict, state_summary: pd.DataFrame = None) -> go.Figure:
    ...
    fig.add_trace(go.Bar(
        ...
        opacity=1.0,
        hovertemplate="Vol: %{customdata[0]:,.0f}<extra></extra>",
        customdata=np.stack([df["Volume"]], axis=-1),
    ), row=2, col=1)
